use gobject_ast::model::{ExportMacro, FunctionDefItem, FileModel, FunctionDeclItem, SourceLocation};
...
impl Rule for GetTypeNotConst {
    ...
    fn check_func_decl(
        ...
        let Some(attr) = Self::find_bad_decl_attr(func) else {
            return;
        };
        ...
        if let Some(fix) = Self::generate_fix(&func.location, attr) {
    ...
    fn check_func_impl(
        ...
        let Some(attr) = Self::find_bad_impl_attr(func) else {
            return;
        };
...
impl GetTypeNotConst {
    fn find_bad_decl_attr(func: &FunctionDeclItem) -> Option<&'static str> {
    ...
    fn find_bad_impl_attr(func: &FunctionDefItem) -> Option<&'static str> {
        panic!("1");
        for &attr in BAD_ATTRS {
            if func.macro_modifiers.iter().any(|m| m == attr) {
                return Some(attr);
    ...
    fn generate_fix(location: &SourceLocation, attr: &str) -> Option<Fix> {
        let decl_text = location.as_str()?;
        let offset = decl_text.find(attr)?;
        let attr_start = location.start_byte + offset;
